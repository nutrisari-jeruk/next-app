...
import { MapSchema } from '@/schemas/income/to-income-sap-13';
...
const fetchList = async ({
  ...
  const data = await $fetch<Pagination<List[]>>({
    method: 'GET',
    url: `/v1/mappings/income-rs/income-sap13?${urlParams}`,
  });
...
const mapOnAccount = async (_prevState: unknown, formData: FormData) => {
  ...
  try {
    if (!!id) {
      await $fetch({
        url: `/v1/mappings/income-rs/income-sap13/${id}`,
        method: 'PUT',
        payload: payload,
      });
    } else {
  ...
  revalidatePath(
    `/mapping/income/to-income-sap-13?mapping=${mapping}&page=${page}`,
  );
  redirect(`/mapping/income/to-income-sap-13?mapping=${mapping}&page=${page}`);
};
