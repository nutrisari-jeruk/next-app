...
const fetchList = async ({
  ...
  const data = await $fetch<Pagination<List[]>>({
    method: 'GET',
    url: `/v1/mappings/financing-050/financing-sap13?${urlParams}`,
  });
...
const mapOnAccount = async (_prevState: unknown, formData: FormData) => {
  ...
  try {
    if (!!id) {
      await $fetch({
        url: `/v1/mappings/financing-050/financing-sap13/${id}`,
        method: 'PUT',
        payload: payload,
      });
    } else {
      await $fetch({
        url: '/v1/mappings/financing-050/financing-sap13',
        method: 'POST',
        payload: payload,
      });
    }
  ...
  revalidatePath(`/mapping/financing-050/to-financing-sap-13?page=${page}`);
  redirect(`/mapping/financing-050/to-financing-sap-13?page=${page}`);
};
