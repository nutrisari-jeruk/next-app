...
import {
  ...
  Params,
  Payload,
} from '@/types/journal-entry/closing';
...
const fetchList = async ({
  ...
  const data = await $fetch<Pagination<List[]>>({
    method: 'GET',
    url: `/v1/transactions/journal-entries/closing?${urlParams}`,
  });
...
const journalKindAutoComplete = async (
  ...
  const data = await $fetch<JournalKindAutoComplete[]>({
    method: 'GET',
    url: `/v1/masters/journals/closing/autocomplete?${urlParams}`,
  });
...
const createJournalEntry = async (_prevState: unknown, formData: FormData) => {
  const validatedFields = ClosingEntrySchema.safeParse({
  ...
  revalidatePath('/transaction/journal-entry/closing');
  redirect(`/transaction/journal-entry/closing`);
};
