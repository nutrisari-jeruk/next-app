...
import {
  ...
  Params,
  Payload,
} from '@/types/journal-entry/correction';
...
const fetchList = async ({
  ...
  const data = await $fetch<Pagination<List[]>>({
    method: 'GET',
    url: `/v1/transactions/journal-entries/correction?${urlParams}`,
  });
...
const journalKindAutoComplete = async (
  ...
  const data = await $fetch<JournalKindAutoComplete[]>({
    method: 'GET',
    url: `/v1/masters/journals/correction/autocomplete?${urlParams}`,
  });
...
const createJournalEntry = async (_prevState: unknown, formData: FormData) => {
  ...
  revalidatePath('/transaction/journal-entry/correction');
  redirect(`/transaction/journal-entry/correction`);
};
