...
import { GeneralEntrySchema } from '@/schemas/transaction/journal-entry/adjustment';
import {
  JournalKindAutoComplete,
  List,
  Params,
  Payload,
} from '@/types/journal-entry/adjustment';
...
const fetchList = async ({
  ...
  const data = await $fetch<Pagination<List[]>>({
    method: 'GET',
    url: `/v1/transactions/journal-entries/adjustment?${urlParams}`,
  });
...
const journalKindAutoComplete = async (
  ...
  const data = await $fetch<JournalKindAutoComplete[]>({
    method: 'GET',
    url: `/v1/masters/journals/adjustment/autocomplete?${urlParams}`,
  });
...
const createJournalEntry = async (_prevState: unknown, formData: FormData) => {
  ...
  try {
    await $http.post('/v1/transactions/journal-entries/adjustment', payload);
  } catch (error) {
  ...
  revalidatePath('/transaction/journal-entry/adjustment');
  redirect(`/transaction/journal-entry/adjustment`);
};
