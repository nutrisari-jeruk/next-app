...
import { PenutupSchema } from '@/schemas/master/journal/closing';
import { redirect } from 'next/navigation';
import { revalidatePath } from 'next/cache';
import { AxiosError } from 'axios';
import type { List, Payload } from '@/types/journal/closing';
...
const fetchList = async ({
  ...
  try {
    ...
    const data = await $fetch<Pagination<List[]>>({
      method: 'GET',
      url: `/v1/masters/journals/closing?${urlParams}`,
    });
...
const createJournal = async (_prevState: unknown, formData: FormData) => {
  ...
  try {
    const data = await $fetch({
      method: 'POST',
      url: `/v1/masters/journals/closing`,
      payload: payload,
    });
  } catch (error) {
    if (error instanceof AxiosError) {
  ...
  revalidatePath('/master/journal/closing');
  redirect(`/master/journal/closing`);
};
